...
require 'json'
require 'digest' # used for SHA256

# Data Structure and some hash functions for credit cards
class CreditCard
  ...
  def self.from_s(card_s)
    ...
    myhash = JSON.parse(card_s)
    CreditCard.new(myhash[:num], myhash[:exp], myhash[:own], myhash[:net])
  end
